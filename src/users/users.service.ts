...
export class UsersService {
    ...
    async updateProfile(userId: number, profileUpdates: any) {
        try {
            const user = await this.userRepo.findOne({ where: { id: userId } });
    ...
    async updatePassword(userId: number, passwordUpdates: updatePasswordDto) {
        try {
            const user = await this.userRepo.findOne({ where: { id: userId } });
            const password = await bcrypt.hash(passwordUpdates.oldPassword, user ? user.salt : '');
            if (user && (user.password == password)) {
                user.password = await bcrypt.hash(passwordUpdates.newPassword, user.salt);
    ...
    async login(loginCreds: LoginUserDto) {
        try {
            const user = await this.userRepo.findOne({ where: { email: loginCreds.email } });
            const password = await bcrypt.hash(loginCreds.password, user ? user.salt : '');
            if (user && (user.password == password)) {
                const payload = {
                    sub: user.id,
                    role: user.role
